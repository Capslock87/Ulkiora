(function() {
  ...
  function mount() {
    ...
    const data = window.__quizData;
    const fromFiles = window.__getTicketsFromFiles && window.__getTicketsFromFiles();
    const tickets = fromFiles || data.applyOverrides(data.generateTickets(), data.manualOverrides);
    let questions;
    if (mode === 'random') {
      const all = data.flattenAllQuestions(tickets);
      questions = data.sampleRandomQuestions(all, 20);
    } else if (ticket) {
      const found = tickets.find(t => t.id === ticket);
      questions = found ? found.questions : tickets[0].questions;
    } else {
